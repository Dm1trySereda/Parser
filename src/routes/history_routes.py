...
from src.repository.api_action.history import RepetitiveBook
from src.response_schemas.history import HistoryOut
from src.validation.book_validates import validate_parameters, validate_searcher
...
async def get_repetitive_books(request: Request):
    ...
    result = [HistoryOut.parse_obj(book.__dict__).dict(by_alias=False) for book in repetitive]
    return result
