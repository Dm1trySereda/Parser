...
class RegistrationUserFacade:
    ...
    async def registration_user(self, new_user: UserRequest) -> UserInDBResponse:
        current_user = await self.search_services.get_current_user(username=new_user.username)
        if current_user is None:
            new_user_record = await self.registration_services.create_new_user(new_user)
        else:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="User with this username already exist"
            )
