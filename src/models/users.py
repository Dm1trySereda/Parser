...
from sqlalchemy import TIMESTAMP, Boolean, ForeignKey, BigInteger, String
...
class AuthProvider(Base):
    ...
    remote_user_id: Mapped[str] = mapped_column(String(length=255), nullable=True, unique=True)
    full_name: Mapped[str] = mapped_column(String(255), nullable=True)
    email: Mapped[str] = mapped_column(
        String(length=320), unique=True, index=True, nullable=False
    )
    user_id: Mapped[int] = mapped_column(ForeignKey("users.id"), nullable=False)
    user_relationship = relationship("User", back_populates="external_user_relationship",
...
class User(Base):
    ...
    external_user_relationship = relationship("AuthProvider", back_populates="user_relationship",
                                              uselist=False)
